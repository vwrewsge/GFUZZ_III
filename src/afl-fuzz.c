...
//   afl->stage_name = "added";
//   afl->stage_short = "added";
...
u8 run_gen_seeds(afl_state_t * afl, u8 init_flag){
  ...
  if (init_flag == 1){
    printf("\n==== Gen start ====\n");
    printf("cp -r %s/all_gen_seeds/%s/* %s/", afl->generator_base_path, afl->target_program, afl->out_dir);
    snprintf(command, sizeof(command), "cp -r %s/all_gen_seeds/%s/* %s/", afl->generator_base_path, afl->target_program, afl->out_dir);
  }else{
    printf("\n==== Gen-M start ====\n");
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while (likely(!afl->stop_soon)) {
    ...
    if (0 && get_cur_time() - afl->last_find_time > 60 * 1000 && get_cur_time() - gen_last_update > 300 * 1000){
